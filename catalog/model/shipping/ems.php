		...
		if ($status) {
			$quote_data = array();
      		...
      		$quote_data['ems'] = array(
        		...
        		'cost'         => $this->config->get('ems_cost'),
        		'tax_class_id' => $this->config->get('ems_tax_class_id'),
				'text'         => $this->currency->format($this->tax->calculate($this->config->get('ems_cost'), $this->config->get('ems_tax_class_id'), $this->config->get('config_tax')))
      		);
