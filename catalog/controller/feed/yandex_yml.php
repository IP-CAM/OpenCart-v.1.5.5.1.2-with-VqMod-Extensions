...
class ControllerFeedYandexYml extends Controller {
	...
	private function generateYml() {
		if ($this->config->get('yandex_yml_status')) {
			...
			if (!($allowed_categories = $this->config->get('yandex_yml_categories'))) exit();
			...
			$products = $this->model_export_yandex_yml->getProduct($allowed_categories, $out_of_stock_id, $vendor_required);
...
//				$data['cbid'] = 15;
				...
				if ($yandex_yml_categ_mapping[$product['category_id']]) {
