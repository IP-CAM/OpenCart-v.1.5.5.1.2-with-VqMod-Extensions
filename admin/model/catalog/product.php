...
class ModelCatalogProduct extends Model {
    ...
    public function getProducts($data = array()) {
        if ($data) {
            $sql = "SELECT p.*, pd.*,  m.name as 'm_name' "
                . "FROM " . DB_PREFIX . "product p "
                . " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)"
                . " LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id)";

            if (!empty($data['filter_weight'])) {
                $sql .= " INNER JOIN " . DB_PREFIX . "product_attribute prod_attr ON p.product_id = prod_attr.product_id "
                    . " AND LCASE(prod_attr.text) LIKE '%" . $this->db->escape(utf8_strtolower($data['filter_weight'])) . "%'";
            }
    ...
    public function getProductAttributeById($product_id, $att_id = PRODUCT_WEIGHT_ID) {
        $query = $this->db->query("SELECT distinct prod_attr.attribute_id, attr_desc.`name`, prod_attr.text as value "
            . "FROM " . DB_PREFIX . "product p "
            . "INNER JOIN " . DB_PREFIX . "product_attribute prod_attr ON p.product_id = prod_attr.product_id "
            . "LEFT JOIN " . DB_PREFIX . "attribute_description attr_desc ON prod_attr.attribute_id = attr_desc.attribute_id "
            . "WHERE p.product_id = '" . (int)$product_id . "' and prod_attr.attribute_id = " . (int)$att_id);
